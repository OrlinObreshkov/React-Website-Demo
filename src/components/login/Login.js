...
function Login() {
  ...
  function submitHandler(e) {
    ...
    if (!email || !password) {
      return dispatch(
        loginSliceReducerActions.setError("All fields required !")
      );
    }

    if (!email.includes("@")) {
      return dispatch(
        loginSliceReducerActions.setError("Email is not valid !")
      );
    }

    if (password.length < 5) {
      return dispatch(
        loginSliceReducerActions.setError("Password is to short !")
      );
    }
